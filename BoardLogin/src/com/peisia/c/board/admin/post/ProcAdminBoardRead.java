	...
	public static void run() {
		...
		try {
			...
			if (DB.result.next()) {
		        ...
		        int report = DB.result.getInt("report");
		        System.out.println("제목: "+title);
		        System.out.println("내용: "+content);
		        System.out.println("조회수: "+hit);
		        System.out.println("신고: "+report);
		    } else {
		...
		loop:
		while(true) {
			String cmd = Ci.r("명령[x:나가기,r:댓글쓰기,p:추천하기,s:삭제하기]");
			switch (cmd) {
			case "x":
				break loop;
			case "r":
			...
			case "p":
				...
				break;
			case "s":
				ProcAdminBoardDelete.run(Integer.parseInt(readNo));
				break;
			default:
