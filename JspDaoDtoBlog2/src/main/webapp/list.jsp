...
<body>
	...
	<%
		...
		if(searchWord == null || searchWord.equals("null")){
			...
			totalPage = dao.getTotalSearchPageCount(searchWord);
		}
	...
	<%
		...
		int currentTotalBlock = (int)Math.ceil((double)nPageNum/Board.List_Amount);
		System.out.println("nPageNum: "+nPageNum);
		System.out.println("currentTotalBlock: "+currentTotalBlock);
		int blockStartNo = (currentTotalBlock - 1)*Board.Page_Link_Amount + 1;
		int blockFinalNo = currentTotalBlock * Board.Page_Link_Amount;
		...
		if(currentTotalBlock == 1){
			hasPrev = false;
		} else{
			hasPrev = true;
			prevPage = (currentTotalBlock - 1) * Board.Page_Link_Amount;
		}
		boolean hasNext = true;
		if(currentTotalBlock < totalBlock){
			hasNext = true;
			nextPage = currentTotalBlock * Board.Page_Link_Amount + 1;
		} else{
			hasNext = false;
		}
		if(hasPrev){
