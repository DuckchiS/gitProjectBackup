	...
	public static void run(int product) {
		loop:
			while(true) {
				...
				switch (KioskObj.cmd) {
				case "1":
					System.out.println("Ahri 캐릭터 " + KioskObj.product.get(product).name + " hot 선택됨. 이전 메뉴 이동");
					KioskObj.basket.add(new Order(KioskObj.product.get(product),1));	//오더 추가
					break loop;
				case "2":
					System.out.println("Timo 캐릭터 " + KioskObj.product.get(product).name + " 선택됨. 이전 메뉴 이동");
					KioskObj.basket.add(new Order(KioskObj.product.get(product),2));	//오더 추가
					break loop;
				case "3":
					System.out.println("Zed 캐릭터 " + KioskObj.product.get(product).name + " 선택됨. 이전 메뉴 이동");
					KioskObj.basket.add(new Order(KioskObj.product.get(product),3));	//오더 추가
					break loop;
				case "x":
